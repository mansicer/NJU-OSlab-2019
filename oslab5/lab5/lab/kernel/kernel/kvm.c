...
SuperBlock sBlock; // super block
GroupDesc gDesc[MAX_GROUP_NUM]; // group descriptor
...
void initDev () {
	...
	dev[1].inodeOffset = inodeOffset; // for STD_IN
}
...
void initFile () {
	...
	for (i = 0; i < MAX_FILE_NUM; i ++) {
		...
		file[i].offset = 0;
		file[i].flags = 0; // O_DIRECTORY, O_CREATE, O_READ, O_WRITE
	}
}
